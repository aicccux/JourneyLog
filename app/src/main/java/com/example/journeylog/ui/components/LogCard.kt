...
fun LogCard(modifier: Modifier, log: Log, formattedDate: String, onDelete: (log: Log) -> Unit) {
    Card(
        ...
        var isDeleteLog by remember { mutableStateOf(false) }
        if (isDeleteLog) {
            DeleteConfirmDialog(onDismiss = { isDeleteLog = false }, log = log, onDelete = onDelete)
        }
        Row(Modifier.padding(8.dp, 0.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            IconButton(onClick = { isDeleteLog = true }) {
                Icon(
                    imageVector = Icons.Filled.Close, contentDescription = "Delete log"
                )
            }
        }
        Row(Modifier.padding(8.dp, 0.dp), verticalAlignment = Alignment.CenterVertically) {
